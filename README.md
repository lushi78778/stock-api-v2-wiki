...
各接口文档中的响应示例均来自 **2026-05-13** 真实行情（本地 API 实调），并按字段拆成 **Markdown 表格** 展示 `data.items`，便于对照字段含义；表格下方仍保留完整 JSON 节选。

| 示例场景 | 交易日 | 说明 |
|----------|--------|------|
| A 股板块、日 K、涨停池等 | 2026-05-13 | 如平安银行 `000001` 收盘 11.14，行业板块 BK1323 等 |
| 外盘商品日 K | 2026-05-15 | 商品库该日有数据时使用 |

若你的环境库内日期不同，把请求里的 `trade_date` / `start` / `end` 换成你有数据的日期即可；也可在仓库中运行 `python3 scripts/refresh_samples.py` 重新拉取并刷新表格。
